    ...
    >
      <div class="nl_l">
        ...
        >
          ...
          <router-link :to="{ path: '/online-complaints' }" class="nl_l_btn">
            <img src="../../assets/img/online.png" />在线投诉处理
          </router-link>
          <router-link :to="{ path: '/firmInfo-search' }" class="nl_l_btn">
            <img src="../../assets/img/menology_h.png" />企业信息查询
          </router-link>
        </el-form>
      </div>
      <div class="nl_r">
        <div class="nl_r_list">
          <h5>
            <i class="iconfont">&#xe6aa;</i> 站内新闻
            <router-link
              :key="$route.fullPath"
              :to="{ name: 'NewsList', query: { id: 1 } }"
              class="btn_more"
              >更多</router-link
            >
          </h5>
          ...
          <ul>
            <li v-for="(item, index) in newsList" v-if="index < 6">
              <b>&nbsp;</b>
        ...
        <div class="nl_r_list nl_r_list_b">
          <h5>
            <i class="iconfont">&#xe714;</i> 会员风采
            <router-link
              :key="$route.fullPath"
              :to="{ name: 'NewsList', query: { id: 2 } }"
              class="btn_more"
              >更多</router-link
            >
          </h5>
          <ul>
            <li v-for="(item, index) in pubList" v-if="index < 5">
              <b>&nbsp;</b>
              <router-link :to="{ name: 'News', params: { id: item.id } }">{{
                item.industryTitle
              }}</router-link>
        ...
        <div class="nl_r_list nl_r_list_b" style="margin-left: 6px;">
          <h5>
            <i class="iconfont">&#xe634;</i> 会员动态
            <router-link
              :key="$route.fullPath"
              :to="{ name: 'NewsList', query: { id: 3 } }"
              class="btn_more"
              >更多</router-link
            >
          </h5>
          <ul>
            <li v-for="(item, index) in hotList" v-if="index < 5">
              <b>&nbsp;</b>
              <router-link :to="{ name: 'News', params: { id: item.id } }">{{
                item.industryTitle
              }}</router-link>
...
export default {
  ...
  methods: {
    ...
    // handleMoreNews() {
    //   console.log("22")
    //   this.$router.push({path:'/newslist',query:{id:1}})
    // },
    refreshCode() {
      this.loginForm.identifyCode = "";
      this.makeCode();
    ...
    handleLogin() {
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          this.loading = true;
          this.$store
            .dispatch("user/login", loginFormEcode)
            .then(() => {
              this.$router.push({
                path: this.redirect || "/",

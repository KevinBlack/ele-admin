<template>
  <el-card class="detailsContainer">
      ...
      <el-row class="area_border">
        ...
        <el-col :span="12">
          <el-form-item label="投诉内容" size="mini" prop="complaintsContents">
            <el-input v-model="formQuery.complaintsContents" size="mini" style="width: 338px !important;"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12" style="text-align: left;padding-left: 100px;">
      ...
      <el-col :span="24">
        <el-button-group size="mini">
          <template slot-scope="scope">
            ...
            <el-button type="primary" size="mini" @click="handleExamine(scope.row)">审核</el-button>
            <el-button type="primary" size="mini" @click="handleProcessLogView(scope.row)">流程日志</el-button>
          </template>
        </el-button-group>
      </el-col>
    </el-row>
    ...
    >
      ...
      <el-table-column prop="complaintsContents" label="投诉内容" :show-overflow-tooltip="true" align="center" />
      <el-table-column prop="complainant" label="投诉人" align="center" />
      <el-table-column prop="complaintName" label="投诉公司" :show-overflow-tooltip="true" align="center"/>
      <el-table-column prop="handlePerson" label="处理人" align="center" :show-overflow-tooltip="true"/>
      <el-table-column align="center" prop="status" :formatter="statusFmt" label="处理状态" />
      <el-table-column prop="remarks" label="备注" align="center" :show-overflow-tooltip="true" />
      <el-table-column prop="auditReason" label="审核描述" align="center" :show-overflow-tooltip="true" />
      ...
      <el-table-column label="相关附件、图片" align="center" width="200">
        <template slot-scope="scope">
          <el-button :key="index" v-for="(item, index) in scope.row.fileList" type="text" @click="fileClick(item.fileCatalog,item.belongId,item.fileName)">{{ item.fileName }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 表格区2end -->
  ...
  methods: {
    ...
    handleAdd() {
      ...
      if (selectRows.length === 0) {
        ...
        return
      } else if (selectRows[0].complaintProfessingId && selectRows[0].examineStatus === '30') {
        this.$message({
          type: 'info',

      ...
      <el-row :gutter="20" class="area_border">
        ...
        <el-col :span="12">
          <el-form-item label="身份标记" size="mini" prop="status">
            <el-select
              v-model="auditForm.identityMark"
              size="mini"
              style="width:100%"
            >
              <el-option
                v-for="item in auditFormOptions.identityMark"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              ></el-option>
...
export default {
  ...
  methods: {
    initForm() {
      getDictDataLists("97001006").then(response => {
        this.auditFormOptions.identityMark = response.data.jq97001006;
      });
      //清空审核原因，防止数据缓存

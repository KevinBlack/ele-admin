...
<script>
import { getMenuList, getMenu, deleteMenu } from "@/api/menu";
...
export default {
  ...
  methods: {
    ...
    handleEdit(row) {
       this.$router.push({ path: "/system/menu-detail", query: {menuId:row.menuId} });
    },
    handleDelete(row) {
    ...
    addSubMnu(row) {
      ...
      this.$router.push({ path: "/system/menu-detail", query: {parentId:parentId} });
    },
